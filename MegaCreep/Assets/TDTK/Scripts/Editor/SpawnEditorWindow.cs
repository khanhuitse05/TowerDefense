    ...
    public class SpawnEditorWindow : EditorWindow
    {
        ...
        Vector2 WaveConfigurator(float startX, float startY)
        {
            ...
            for (int i = 0; i < instance.waveList.Count; i++)
            {
                ...
                EditorUtilities.DrawSprite(new Rect(startX, startY - 2, 20, 20), EditorDBManager.iconRsc);
                wave.rscGain = EditorGUI.IntField(new Rect(startX + 21, startY, 40, height - 2), wave.rscGain);
        ...
        Vector2 SubWaveConfigurator(Wave wave, float startX, float startY, float spaceX, float width, float spaceY, float height)
        {
            ...
            for (int i = 0; i < wave.subWaveList.Count; i++)
            {
                ...
                GUI.Box(new Rect(startX - 5, startY + spaceY - 5, spaceX + width2 + 10, 7 * spaceY + 30), "");
                ...
                {
                    subWave.path = instance.defaultPath;
                }
        ...
        Vector2 WaveGeneratorSetting(float startX, float startY)
        {
            ...
            EditorUtilities.DrawSprite(new Rect(startX, startY - 5, 22, 22), EditorDBManager.iconRsc);
            cont = new GUIContent("     GOLD:");
            v2 = DrawProceduralVariable(startX, startY, waveGen.rscSetting, cont);
        ...
        void OnGUI()
        {
            ...
            cont = new GUIContent("develop: ", "HP = defaultHp + defaultHp * devPerWave * indexWave");
            EditorGUI.LabelField(new Rect(startX, startY += spaceY, width + 50, height), cont);
            instance.developPerWave = EditorGUI.FloatField(new Rect(startX + spaceX, startY, width, height), instance.developPerWave);
