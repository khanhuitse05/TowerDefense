...
public class ObjectPoolManager : MonoBehaviour{
	...
	public static int New(Transform objT, int cap=2){ if(instance==null) Init(); return instance._New(objT.gameObject, cap); }
	public static int New(GameObject obj, int cap=2){ if(instance==null) Init(); return instance._New(obj, cap); }
	public int _New(GameObject obj, int cap=1000){
		int ID=GetPoolID(obj);
		
		if(ID!=-1) poolList[ID].MatchObjectCapacity(cap);
		else{
			Pool pool=new Pool();
			pool.prefab=obj;
			pool.MatchObjectCapacity(cap);
			poolList.Add(pool);
			ID=poolList.Count-1;
		}
		return ID;
...
public class Pool
{
    ...
    public GameObject Spawn(Vector3 pos, Quaternion rot)
    {
        ...
        else if (GetTotalObjectCount() < capacity)
        {
    ...
    public void MatchObjectCapacity(int count)
    {
        capacity = count;
    }
