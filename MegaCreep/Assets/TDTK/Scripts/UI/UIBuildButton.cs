...
namespace TDTK
{
    public class UIBuildButton : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int i = 0; i < towerList.Count; i++)
            {
                if (i == 0)
                    ...
                    buttonList.Add(buttonList[0].Clone("button" + (i + 1), new Vector3(i * -150, 0, 0)));
                buttonList[i].imageIcon.sprite = towerList[i].iconSprite;
                buttonList[i].label.text = "" + towerList[i].GetBuildCost();
        ...
        public void OnTowerButton(GameObject butObj)
        {
            ...
            if (exception == false) UIGameMessage.DisplayMessage("Can't build tower");
        ...
        void Update()
        {
            if (!UI.UseDragNDrop() && activeButtonList.Count > 0)
            {
                float cutoff = 0;
                Vector3 screenPos = Camera.main.WorldToViewportPoint(buildInfo.position);
                Vector2 canvasPos = new Vector2 
                    (
                        screenPos.x * Screen.width,
                        screenPos.y * Screen.height
                    );
                List<Vector3> pos = GetPieMenuPos(activeButtonList.Count, canvasPos, cutoff, 150);

                for (int i = 0; i < activeButtonList.Count; i++)
                {
                    if (i < pos.Count)
                    {
                        activeButtonList[i].rootObj.transform.localPosition = pos[i];
                    }
                }
            }
        }
        ...
        public static List<Vector3> GetPieMenuPos(float num, Vector3 screenPos, float cutoff, int size)
        {
            ...
            if (num == 1)
            {
                points.Add(screenPos + new Vector3(0, 150, 0));
                return points;
            }
            //if there's only two button to be displayed, then normal calculation doesnt apply
            if (num <= 2)
            {
                points.Add(screenPos + new Vector3(150, 10, 0));
                points.Add(screenPos + new Vector3(-150, 10, 0));
                return points;
            }
