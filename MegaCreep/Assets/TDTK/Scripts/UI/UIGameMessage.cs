	...
	public class UIGameMessage : MonoBehaviour {
		...
		void _DisplayMessage(string msg){
			...
			foreach(GameObject msgObj in msgList){
				Vector3 pos=txtGameMessage.transform.localPosition+new Vector3(0, counter*35, 0);
				TweenPosition(msgObj, .15f, pos);
				counter-=1;
			}
			
			GameObject obj=(GameObject)Instantiate(txtGameMessage);
			obj.transform.SetParent(txtGameMessage.transform.parent);
			obj.transform.localPosition=txtGameMessage.transform.localPosition;
			obj.transform.localScale=txtGameMessage.transform.localScale;
