            ...
            while (Input.touchCount >= 1)
            {
                ...
                if (Input.touches[0].phase == TouchPhase.Ended)
                {
                    ...
                    if (status == _TileStatus.Available)
                    {
                        ...
                        if (exception == false) GameControl.DisplayMessage("Can't build tower");
                    }
                    else
                    {
        ...
        public int GetCost(int ID = 0)
        {
            ...
            else
            {
                ...
                if (currentActiveStat < stats.Count - 1)
                    cost = stats[currentActiveStat + 1].cost;
                else if (ID < nextLevelTowerList.Count && nextLevelTowerList[ID] != null)
                    cost = nextLevelTowerList[ID].stats[0].cost;
            }
            cost = (int)Mathf.Round(cost * multiplier);
            return cost;
        }
